...
// Case 2: Cannot be hoisted even it's declared with "var" keyword
var bar = function () {
  console.log("bar!");
};
```
...
With let (and const), the declaration is hoisted too, but not initialized.
The variable is placed in memory, but it stays in the Temporal Dead Zone (TDZ) until the actual declaration line is executed.

Hoisting doesn’t literally move code to the top. Instead, during the creation phase of the execution context, the JavaScript engine allocates memory for variables and functions. With var, variables are hoisted and initialized immediately with undefine whereas let/const - they are placed in memory but remain in the Temporal Dead Zone (TDZ) until the declaration line is executed.

```pre
Global Scope Start
|
| TDZ begins (x is hoisted but uninitialized)
|
| console.log(x)
|
| let x; TDZ ends, x initialized to undefined
|
Global Scope End
```
