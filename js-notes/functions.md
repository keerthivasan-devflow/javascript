...
- Parameters - The values that are received inside function declaration are called as "parameters"
- Arguments - The values that are passed inside the function call are called as "arguments".
...
Closures are the combination of a function bundled together with references to its surrounding state (lexical environment). _[A closures function gives access to an outer function’s scope from an inner function.]_
